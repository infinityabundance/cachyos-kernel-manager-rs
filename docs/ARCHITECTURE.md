...
| 11 | Boot/system courts | in progress | real kernel mutations, reboot, residual comparison. Done: boot/system-boot-after-install PASS (VM: a REAL linux-cachyos-lts install via the courted command (literal == the model render) runs the REAL post-install hooks (mkinitcpio regenerates /boot with the new initramfs), the same overlay REBOOTS, the system comes up boot-complete, the mutation persisted; the two boots' surfaces byte-identical; the kernel-install fixture caches the real lts packages offline). Remaining: the removal court (remove a NON-running kernel + reboot + the bootloader entry gone), the failed-boot residual comparison, and the multi-reboot drift runs. |
