...
| 12 | Hostile review | in progress | Phase 12 restructured (audit 2026-08-23): FIRST the production-integration closure (ui/gui-drive --vm — drive the PACKAGED binary, not witness CLIs), then hostile review: security, fuzzing, race/mutation testing, dependency audit, unexplained residual hunt; production privilege replacement (D-001). The audit P-list (P0 build-guard/cancel/SCX model seams, D-003 default-build grammar; P1 patch-refresh generation, per-child build env, fail-closed background tasks, transaction-boundary TOCTOU refresh, AUR executor cwd/~-expansion/stale-checkout, O_EXCL probe files, chwd dependency; P2 scx show-not-toggle, empty patch submission guards, D-009 licensing/MSRV) is IMPLEMENTED + tested. The production-integration slice is CLOSED: ui/gui-drive PASS (2026-08-23, fixture gui-integration — the packaged binaries driven through sort-every-column → toggle identity proof; the court caught TWO candidate divergences: the discovery order — the oracle's alpm_db_search vs the candidate's pkgcache iteration (FFI now binds alpm_db_search) — and the sort semantics — Qt's _q_headerClicked resets a new column to ascending and re-sorts the displayed items (fixed + unit-tested)). The hostile-review rendered-i18n court ui/i18n-rendered PASS (fixture i18n-rendered, generated de_DE.UTF-8 + zh_CN.UTF-8 — the rendered chrome projection (title/description/headers/buttons) byte-identical per locale; the de_DE surface witnesses the audit P2 tr() fixes (description + headers + variant-label context + the qtbase-derived standard dialog buttons), the zh_CN surface witnesses gap-009's rendered projection (both sides English)). The gap-010 close-during-transaction race — the audit's explicit race-hunt mandate — is REPRODUCED and courted: ui/close-during-transaction PASS (fixture close-transaction with a slow-pacman in-flight window; the frozen Qt app ABORTS on close-during-transaction — SIGABRT, Qt's QThread-destroyed-while-running after closeEvent's alpm_release — the release Slint binary exits CLEANLY; the machine residuals match byte-for-byte — the close corrupts nothing on either side; D-008 INTENTIONAL_CORRECTION). Full VM matrix re-run: 40 VM courts PASS (fail-closed aggregator vm/run-vm-matrix.sh). SECURITY.md carries the audit-checklist dispositions. Remaining Phase 12: the production privilege replacement (D-001, the narrow typed helper + polkit shim — Phase 13 implementation work) and the dependency audit. |
