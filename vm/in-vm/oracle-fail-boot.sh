#!/usr/bin/env bash
#
# oracle-fail-boot.sh — the ORACLE side of the boot/system-boot-after-
# failure court (Phase 11), phase 1: SETS UP the two-kernel state
# (installs the cached lts), then REMOVES the RUNNING kernel (the base
# linux-cachyos + headers — the kernel the qemu direct boot loads) with
# the frozen source's literal command (`pacman -Rsn linux-cachyos
# linux-cachyos-headers` — commit_transaction, kernel.cpp:288-304) under
# strace, lets the post-remove hooks (mkinitcpio) run, and records the
# pre/post state. The reboot phase (boot-check-failure.sh) witnesses the
# FAILED state: the machine's own boot path is destroyed (the running
# kernel's packages + /boot entry GONE), while the harness's direct-kernel
# boot still brings the machine up for the residual.
#
set -euo pipefail
OUT="$1"
mkdir -p "$OUT"

REMOVE_LITERAL='pacman -Rsn linux-cachyos linux-cachyos-headers'
...
# the post-remove state (the hooks ran: the base /boot entries gone)
