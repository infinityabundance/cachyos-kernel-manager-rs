...
| 10 | Packaging and migration | in progress | Arch package, drop-in files, package replacement, upgrade/revert courts. Done: packaging/PKGBUILD (builds the Rust GUI with gui-alpm + installs the byte-identical drop-in files), packaging/file-layout PASS (the candidate's installed file set == the frozen oracle package's 15-file surface, the 14 shared files byte-identical, sha256:3dd688c6... oracle package preserved in oracle/packages/), packaging/upgrade PASS (VM: oracle 1.19.0-1 -> candidate 0.1.0-1 -> oracle 1.19.0-1; the file surface + discovery rows preserved across the transition; the oracle's --version abort quirk documented). Remaining: the packaging/README + the migration docs, the full packaging court matrix in the forensic workflow, and the Phase 11 boot/system courts follow. |
