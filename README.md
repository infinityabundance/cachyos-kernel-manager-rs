...
| 9 | Full differential court matrix | in progress | failure paths, historical regressions, repeated executions, drift/slew. Done: single-instance/stale-lock PASS (gap-001 covered), drift-slew/pure-determinism PASS (all pure witnesses x3 fresh-process runs byte-identical), kernel-discovery/needle-order PASS (gap-002 covered: the adversarial needle db row set + ORDER witnessed identical between the oracle's alpm_db_search and the candidate's pkgcache discovery on the same libalpm; the order remains a libalpm-internal property, not an ALPM contract). Remaining: the makepkg -scf vs -sicf runtime dependency-resolution witness (gap-006), historical-regression courts, and the close-during-transaction race witness (gap-010). |
