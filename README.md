...
| 9 | Full differential court matrix | in progress | failure paths, historical regressions, repeated executions, drift/slew. Done: single-instance/stale-lock PASS (gap-001 covered), drift-slew/pure-determinism PASS (all pure witnesses x3 fresh-process runs byte-identical), kernel-discovery/needle-order PASS (gap-002 covered: the adversarial needle db row set + ORDER witnessed), build-env/makepkg-runtime PASS (gap-006 covered: real makepkg runtime witness — -scf resolves repo deps via pacman -S --asdeps + touch .done-status without installing; -sicf adds the pacman -U install; an AUR-only dep fails -s identically; the candidate's model-rendered commands == the frozen literals). Remaining: historical-regression courts and the close-during-transaction race witness (gap-010). |
